...
Its read-only transcript status and memory validation collectors also allow one bounded delayed retry after a timeout so concurrent due jobs on agent launch do not create a false weekly-audit failure.
