...
SMART_WAKE_CODE_DIR="${SMART_WAKE_CODE_DIR:-$(cd "$(dirname "$0")" && pwd -P)}"
# shellcheck disable=SC1091
source "${SMART_WAKE_CODE_DIR}/lib.sh"
...
smart_wake_cycle() {
    ...
    now=$(now_epoch)
    lid_trigger_grace_until=$(read_epoch_file "$LID_TRIGGER_GRACE_FILE")
    lid_trigger_grace=false
    ac=false
    ac_loss_grace=false
    disabled=false
    trusted=false
    ...
    if [ -n "$lid_trigger_grace_until" ] &&
        [ "$lid_trigger_grace_until" -gt "$now" ] 2>/dev/null; then
        lid_trigger_grace=true
    fi
    ...
    if [ "$keep_awake" = false ] && [ "$lid_trigger_grace" = true ]; then
        # The root helper owns this short window so a delayed Wi-Fi, AC, or
        # manual-override signal can appear without extending the normal
        # cooldown or sleep-guard lease.
        stop_caffeinate
        clear_sleep_guard_lease
        rm -f "$LAST_ALLOWED_FILE" "$ALLOWED_LOSS_SINCE_FILE" \
            "$COOLDOWN_UNTIL_FILE" "$COOLDOWN_NOTIFIED_FILE"
        reason="waiting for an allowed condition until $(format_epoch "$lid_trigger_grace_until")"
        keep_awake=true
        handle_lid_lock true
        write_status "$now" "$keep_awake" "$reason" "$mode" "$ssid" "$trusted" "$ac" "$battery" "${override_until:-}" ""
        return 0
    fi
    ...
    if [ "$disabled" = true ] && [ "$ac" = false ] && [ "$ac_loss_grace" = false ]; then
        ...
        if ! clamshell_closed; then
            ...
            if [ "$((now - loss_since))" -ge "$TRUSTED_WIFI_LOSS_GRACE_SECONDS" ]; then
                ...
                if [ -z "$ssid" ]; then
                    keep_awake=false
                    reason="Wi-Fi disconnected after reconnection grace"
                    stop_caffeinate
                    rm -f "$LAST_ALLOWED_FILE" "$COOLDOWN_UNTIL_FILE" "$COOLDOWN_NOTIFIED_FILE"
                    cooldown_until=""
                    log_msg "Wi-Fi remained disconnected after ${TRUSTED_WIFI_LOSS_GRACE_SECONDS}s; allowing sleep"
                else
                    start_cooldown "$now"
                    cooldown_until=$(read_epoch_file "$COOLDOWN_UNTIL_FILE")
                    reason="cooldown until $(format_epoch "$cooldown_until")"
