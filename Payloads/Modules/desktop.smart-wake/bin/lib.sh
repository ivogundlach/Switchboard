...
SMART_WAKE_CODE_DIR="${SMART_WAKE_CODE_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)}"
...
DISCORD_WEBHOOK_FILE="${SMART_WAKE_WEBHOOK_FILE:-${SMART_WAKE_HOME}/credentials/webhook}"
DISCORD_REPLY_WINDOW_FILE="${STATE_DIR}/discord-reply-window.json"
DISCORD_WATCHER_LABEL="com.user.smartwake.discord"
...
load_config() {
    ...
    : "${DISCORD_REPLY_WINDOW_SECONDS:=1800}"
...
current_ssid() {
    ...
    "${SMART_WAKE_CODE_DIR}/wifi-ssid.py" 2>/dev/null
}
...
send_discord() {
    local body="$1"
    local webhook_url payload http_code response_file message_id now deadline
...
PY
    ...
    response_file=$(mktemp "${STATE_DIR}/discord-send.XXXXXX") || return 75
    ...
    if [ "$http_code" = "200" ]; then
        message_id=$(/usr/bin/python3 - "$response_file" <<'PY'
import json
import sys
try:
    value = json.load(open(sys.argv[1], encoding="utf-8")).get("id", "")
    print(value if str(value).isdigit() else "")
...
PY
        ...
        if [ -z "$message_id" ]; then
            log_msg "Discord notification response did not include a message ID"
            return 75
        fi
        now=$(now_epoch)
        deadline=$((now + DISCORD_REPLY_WINDOW_SECONDS))
        /usr/bin/python3 - "$DISCORD_REPLY_WINDOW_FILE" "$message_id" "$deadline" <<'PY'
import json
import os
import sys
path, message_id, deadline = sys.argv[1:]
temporary = path + ".tmp"
with open(temporary, "w", encoding="utf-8") as handle:
    json.dump({"messageID": message_id, "deadline": int(deadline)}, handle,
