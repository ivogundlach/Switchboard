...
PROGRAM = Path(os.environ.get("WSI_PROGRAM", HOME / ".local/bin/weekly-system-improvement"))
PROTECTED = [
...
def main() -> int:
    ...
    label = f"{os.environ.get('WSI_LAUNCHD_LABEL', 'com.example.weekly-system-improvement')}-acceptance-{os.getpid()}"
    loaded = False
    succeeded = False
    launch_snapshot = ""
